...
Query.prototype.send = function (method, url, options) {
    if(log_debug) { var _req_id = Math.random() }
    var _callback = this.callback_f
    var buffer = ""
    if(log_debug) { sys.log("[http log]["+_req_id+"]["+method+"]"+url) }
    var request = this.client.request(method, url, options) 
        .addListener('response', function(response) {
            if(response.statusCode != 200) {
                if(log_debug) { sys.log("[http log]["+_req_id+"][response]"+response.statusCode) }
                _callback(true, null)
            } else {
                ...
                response.addListener('data', function(chunk) {
                    ...
                    if(log_debug) { sys.log("[http log]["+_req_id+"][response body]"+buffer) }
    ...
    var first = true
    if(log_debug) { var log_request_body = "" }
    // record data
    if(typeof(this._properties)!= 'undefined') {
        for(var p in this._properties) {
            if(!first) {
                request.write("&")
                if(log_debug) { log_request_body += "&" }
            }
            request.write(p + "=" + this._properties[p])
            if(log_debug) { log_request_body += p + "=" + this._properties[p] }
            first = false
        }
    }
    // record as location
    if(typeof(this.latitude)!= 'undefined' && typeof(this.longitude)!= 'undefined') {
        ...
        if(log_debug) { log_request_body += "_loc=" + this.latitude + "," + this.longitude }
    }
    if(log_debug) { sys.log("[http log]["+_req_id+"][request body]" + log_request_body) }
    request.end()
}
 ...
 * @param {Hash} options use 'limit' as ineteger to max number of result and 'properties' as 0/1 to include or not properties to the response.
 * @param {Function} callback (err, hash) to process the response.
 */
Record.prototype.nearby = function(radius, options, callback) {
    ...
    if(typeof(options) == 'object') {
        for(var o in options) {
            url += "&" + encodeURI(o) + "=" + encodeURI(options[o])
        }
    }
    sys.log(url)
    new Query(this.layer.pincaster)
...
/**
 * set the record as expirable.
 * @param {Integer} timestamp when does the record will expired (use empty string or 0 to revert).
 * @param {Function} callback (err, ack)
 */
Record.prototype.expire = function (timestamp, callback) {
    new Query(this.layer.pincaster)
        .callback(callback)
        .properties({_expires_at: timestamp})
        .put('/api/1.0/records/' + encodeURI(this.layer.name) + '/' + encodeURI(this.key) + '.json', {'content-type': 'application/x-www-form-urlencoded'})
}
